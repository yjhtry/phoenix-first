defmodule HeadsUpWeb.AdminIncidentLive.Form do
  ...
  def submit_incident(socket, :new, params) do
    case Admin.add_incident(socket.assigns.incident, params) do
      {:ok, _incident} -> 
        socket = socket
  ...
  def render(assigns) do
    ...
    <.simple_form for={@form} id="incident-form" phx-submit="submit" phx-change="validate">
      ...
      <.input phx-debounce="blur" label="description" type="textarea" field={@form[:description]} autocomplete="off" />
      <.input label="Priority" type="number" field={@form[:priority]} min="1" />
      <.input label="status" type="select" field={@form[:status]} options={Incidents.status()}  />
      <.input label="Image Path" type="text" field={@form[:image_path]} value="/images/placeholder.jpg" />
      <:actions>
